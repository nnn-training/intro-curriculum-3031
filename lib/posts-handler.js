...
function addTrackingCookie(cookies, userName) {
  ...
  if (isValidTrackingId(requestedTrackingId, userName)) {
    ...
    const randomNum = parseInt(crypto.randomBytes(8).toString('hex'), 16);
    //const originalId = Math.floor(randomNum * Number.MAX_SAFE_INTEGER);    
    const originalId = randomNum;
    const tomorrow = new Date(Date.now() + (1000 * 60 * 60 * 24));
    const trackingId = originalId + '_' + createValidHash(originalId, userName);
    cookies.set(trackingIdKey, trackingId, { expires: tomorrow });
    console.log(`randomNum:${randomNum}, originalId:${originalId}`);
    return trackingId;
  }
}
